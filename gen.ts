...
function lerp(a: string, b: string, t: number, u = 1 - t): string {
  return toHex(
    fromHex(a).map((x, i) =>
      Math.sqrt((x ** 2) * u + (fromHex(b)[i] ** 2) * t)
    ) as never,
  );
}
...
const baseTokens = [
  ...
  scope("keyword.operator.key-value, keyword.operator.namespace, keyword.operator.access.dot", white),
...
const theme = {
  ...
  colors: {
    ...
    "editor.lineHighlightBackground": darkergrey,
  ...
  "tokenColors": [
    ...
    { scope: "entity.other.attribute-name.id", settings: { fontStyle: "bold" } },
    { scope: "punctuation.definition.entity.css entity.other.attribute-name.id, keyword.other.important.css", settings: { fontStyle: "bold" } },
    { scope: "heading", settings: { fontStyle: "bold", foreground: blue } },
    ...baseTokens,
    scope("meta.embedded.block.rustdoc", comment),
    ...baseTokens.map((x) =>
      scope(
        x.scope.split(", ").map((x) => "meta.embedded.block.rustdoc " + x).join(
          ", ",
        ),
        lerp(
    ...
    scope("meta.embedded.block.rustdoc comment", comment),
    { scope: "meta.embedded.block.rustdoc heading", settings: { fontStyle: "bold", foreground: comment } },
  ],
};
